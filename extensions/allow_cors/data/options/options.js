...
var config = {
    ...
    "load": function () {
        ...
        config.elements.add.addEventListener("click", function () {
            config.listener.add('')
        });
        /*  */
        ...
        "save": function (e) {
            ...
            if (hostname) {
                if (origin || methods || headers || credentials) {
                    ...
                    try {
                        hostname = hostname ? (new URL(hostname)).hostname : hostname
                    } catch (e) {
                    }
                    try {
                        methods = methods ? methods.split(',').map(e => e.trim()).filter(e => e).join(", ") : methods
                    } catch (e) {
                    }
                    try {
                        headers = headers ? headers.split(',').map(e => e.trim()).filter(e => e).join(", ") : headers
                    } catch (e) {
                    }
                    //
